  ...
  ({ className, variant = 'default', decoration = 'none', rotate = 0, shadow = 'md', children, ...props }, ref) => {
    ...
    return (
      ...
      >
        {decoration === 'tape' && (
          <>
            {/* Top Left Tape - Miring */}
            <div className="absolute -top-4 -left-8 w-24 h-8 bg-yellow-100/40 backdrop-blur-[1px] rotate-[-35deg] z-10 border-x border-foreground/5 shadow-sm" />
            
            {/* Bottom Right Tape - Menyilang */}
            <div className="absolute -bottom-4 -right-8 w-24 h-8 bg-yellow-100/40 backdrop-blur-[1px] rotate-[-35deg] z-10 border-x border-foreground/5 shadow-sm" />
          </>
        )}
        {decoration === 'tack' && (
